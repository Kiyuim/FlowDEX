...
func (m customPairModel) FindLatestCompletingPumpLimit(ctx context.Context, pageNum, pageSize int32) ([]Pair, error) {
	resp := make([]Pair, 0)
	err := m.conn.WithContext(ctx).Model(&Pair{}).Where("name = ? and pump_status = 1", constants.PumpFun).Order("created_at  desc").Offset(int((pageNum - 1) * pageSize)).Limit(int(pageSize)).Find(&resp).Error
	return resp, err
}
