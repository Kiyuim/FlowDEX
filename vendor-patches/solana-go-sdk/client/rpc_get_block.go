...
func (c GetBlockConfig) toRpc() rpc.GetBlockConfig {
	return rpc.GetBlockConfig{
		...
		// Patched: upstream hardcodes 0, which rejects any block containing a
		// version-1 transaction ("Transaction version (1) is not supported").
		// See docs/项目已知问题与修复记录.md.
		MaxSupportedTransactionVersion: pointer.Get[uint8](1),
	}
}
